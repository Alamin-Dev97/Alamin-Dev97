...
module.exports = {
  config: {
    ...
    longDescription:
      "Use '/video [name]' to search or '/video -v [YouTube URL]' for video download or '/video -a [YouTube URL]' for audio download.",
    category: "MUSIC",
    guide:
  ...
  onStart: async function ({ api, event, args }) {
    ...
    try {
      ...
      if (durationSeconds > 600) {
        throw new Error(
          `This video is too long (${timestamp}). Only videos under 10 minutes are supported.`
        );
      }
      ...
      const safeTitle = topResult.title
        .replace(/[\\/:*?"<>|]/g, "")
        .substring(0, 50);
